...
[eggi.sh](files/eggi.sh)

### Solution

The shell script was obfuscated as expected.
...
I focused on the `$Ax2$xTT` at the beginning of the third row. I searched for variables `Ax2` and `xTT` in the code and it turned out that `$Ax2$xTT` is just an obfuscated transcript of `eval`.

I replaced it with `echo` and ran the script. The result was much more readable.
...
Now it was clear what arguments should be passed to the script. However it was simpler to strip all the validation code and let the script print the result directly. This is what was left.
